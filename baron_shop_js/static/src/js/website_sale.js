...
$('.oe_website_sale').each(function () {
    ...
    $(oe_website_sale).on('click', 'a.js_add_cart_json', function (ev) {
        ...
        if (quantity > 0) {
            ...
            var uom_val = 1;
            var uos_val = 1;
            $("input.js_variant_change:checked").siblings("span[name=attr_uom_val]").each(function (index, el) {
                uos_val = uos_val * parseFloat($(el).text() || 1);
            });
            ...
            var res_uos_qty = uos_qty * uos_val;
            var res_uom_qty = res_uos_qty / (uos_coeff * uos_qty);
            ...
            for (var k in variant_ids) {
                if (_.isEmpty(_.difference(variant_ids[k][1], values))) {
                    ...
                    var total_price = result_price * quantity * res_uom_qty;
